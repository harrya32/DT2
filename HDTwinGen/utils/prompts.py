...
OFFLINE_CONTROL_PROMPT_SPECS = {
    'Dataset-LunarLander': {
        'state_dim': 8,
        'action_dim': 2,
        'state_range': (-2.4808201789855957, 4.900119781494141),
    ...
    'Dataset-Cheetah': {
        'state_dim': 17,
        'action_dim': 6,
        'state_range': (-28.294265747070312, 38.473114013671875),
        'action_range': (-1.0, 1.0),
    },
    'Dataset-Ant': {
        'state_dim': 27,
        'action_dim': 8,
        'state_range': (-18.71476936340332, 18.52329444885254),
        'action_range': (-1.0, 1.0),
    },
}

def _build_channel_description(prefix, count, meaning):
    lines = []
    for idx in range(count):
        lines.append(f"* {prefix}_{idx}: {meaning} (channel {idx})")
    return '\n'.join(lines)
...
"""
        ...
        return """Learning Dynamics from an Offline Torque-Controlled Rotational System Dataset

Here you must model the state differential of cos_theta, sin_theta, and theta_dot; with the input action torque. The dataset is an offline control dataset and contains trajectory windows extracted from logged transitions.

Description of the variables:
* cos_theta: cosine of the pendulum angle
* sin_theta: sine of the pendulum angle
* theta_dot: angular velocity of the pendulum
* torque: control torque applied by the agent

The time unit is one simulation step.

Additionally these variables have approximate ranges of:
* cos_theta: [-1, 1]
* sin_theta: [-1, 1]
* theta_dot: approximately [-8, 8]
* torque: approximately [-2, 2]

The training dataset consists of fixed-length windows sampled from offline trajectories. Do not assume any known benchmark equations; infer dynamics from data."""
    elif env_name in OFFLINE_CONTROL_PROMPT_SPECS:
        spec = OFFLINE_CONTROL_PROMPT_SPECS[env_name]
        state_desc = _build_channel_description(
            prefix='state',
            count=spec['state_dim'],
            meaning='continuous state variable inferred from logged trajectories',
        )
        action_desc = _build_channel_description(
            prefix='action',
            count=spec['action_dim'],
            meaning='continuous control input applied at each step',
        )
        return f"""Learning Dynamics from an Offline Controlled System Dataset

Here you must model the state differential of the variables state_0 through state_{spec['state_dim'] - 1}, with input actions action_0 through action_{spec['action_dim'] - 1}. The dataset contains fixed-length trajectory windows extracted from logged transitions.
...
Additionally these variables have approximate ranges:
* each state_i: approximately [{spec['state_range'][0]:.3g}, {spec['state_range'][1]:.3g}]
* each action_i: approximately [{spec['action_range'][0]:.3g}, {spec['action_range'][1]:.3g}]

Important: treat this as an unknown controlled dynamical system and infer dynamics only from data. Do not assume a known benchmark transition equation."""
    if env_name == 'COVID':
...
def get_skeleton_code(env_name):
    ...
    def forward(self, cos_theta: torch.Tensor, sin_theta: torch.Tensor, theta_dot: torch.Tensor, torque: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        # TODO: Fill in the code here
        return (d_cos_theta__dt, d_sin_theta__dt, d_theta_dot__dt)"""
    elif env_name in OFFLINE_CONTROL_PROMPT_SPECS:
        spec = OFFLINE_CONTROL_PROMPT_SPECS[env_name]
        return f"""class StateDifferential(nn.Module):
    def __init__(self):
        super(StateDifferential, self).__init__()
        # TODO: Fill in the code here

    def forward(self, state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        # state has shape (batch_size, {spec['state_dim']})
        # action has shape (batch_size, {spec['action_dim']})
        # return d_state__dt with shape (batch_size, {spec['state_dim']})
        # TODO: Fill in the code here
        return d_state__dt"""
    if env_name == 'COVID':
        return """class StateDifferential(nn.Module):
...
* The input tensors follow the skeleton signature and are batched by the first dimension (batch_size).
