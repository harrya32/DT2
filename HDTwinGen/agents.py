...
self.re_tries)
    ...
    def generate_reflection_competition_for_generation_dict(self, generation_dict):
        loss_per_dim_dict = generation_dict['code_dict'].get('loss_per_dim_dict', {})
        if self.env.env_name == 'Cancer' or self.env.env_name == 'Cancer-ood' or self.env.env_name == 'Cancer-iid' or 'Cancer-random' in self.env.env_name:
            val_loss_per_dim_str = f"(Where the val loss per dimension is tumor_volume val loss: {loss_per_dim_dict['tumor_volume_val_loss']:.3g}, chemotherapy_drug_concentration val loss: {loss_per_dim_dict['chemo_drug_concentration_val_loss']:.3g})"
        elif self.env.env_name == 'Cancer-untreated':
            val_loss_per_dim_str = f"(Where the val loss per dimension is tumor_volume val loss: {loss_per_dim_dict['tumor_volume_val_loss']:.3g})"
        elif self.env.env_name == 'Cancer-chemo':
            val_loss_per_dim_str = f"(Where the val loss per dimension is tumor_volume val loss: {loss_per_dim_dict['tumor_volume_val_loss']:.3g}, chemotherapy_drug_concentration val loss: {loss_per_dim_dict['chemo_drug_concentration_val_loss']:.3g})"
        elif self.env.env_name == 'Dataset-3DLV':
            val_loss_per_dim_str = f"(Where the val loss per dimension is prey_population val loss: {loss_per_dim_dict['prey_population']:.3g}, intermediate_population val loss: {loss_per_dim_dict['intermediate_population']:.3g}, top_predators_population val loss: {loss_per_dim_dict['top_predators_population']:.3g})"
        elif self.env.env_name == 'Dataset-HL':
            val_loss_per_dim_str = f"(Where the val loss per dimension is hare_population val loss: {loss_per_dim_dict['hare_population']:.3g}, lynx_population val loss: {loss_per_dim_dict['lynx_population']:.3g})"
        elif self.env.env_name == 'Dataset-Pendulum':
            val_loss_per_dim_str = f"(Where the val loss per dimension is cos_theta val loss: {loss_per_dim_dict['cos_theta']:.3g}, sin_theta val loss: {loss_per_dim_dict['sin_theta']:.3g}, theta_dot val loss: {loss_per_dim_dict['theta_dot']:.3g})"
        elif self.env.env_name == 'COVID':
            val_loss_per_dim_str = f"(Where the val loss per dimension is susceptible val loss: {loss_per_dim_dict['susceptible']:.3g}, exposed val loss: {loss_per_dim_dict['exposed']:.3g}, infected val loss: {loss_per_dim_dict['infected']:.3g}, recovered val loss: {loss_per_dim_dict['recovered']:.3g})"
        else:
            dim_parts = [f"{key} val loss: {loss_per_dim_dict[key]:.3g}" for key in sorted(loss_per_dim_dict.keys())]
            if dim_parts:
                val_loss_per_dim_str = f"(Where the val loss per dimension is {', '.join(dim_parts)})"
            else:
                val_loss_per_dim_str = ''
        completion = f"""
...
""", 'function_call': {"name": 'complete_StateDifferential_code'}})
        ...
        best_program_path = f'{self.folder_path}best_program.json'
        best_program_source_path = f'{self.folder_path}best_state_differential.py'
        ...
        with open(best_program_source_path, 'w') as f:
            f.write(best_program['StateDifferential_code'])
        self.logger.info(f'[{self.env.env_name} {self.name} {self.env.seed}][Export] Saved best program to {best_program_path}')
        self.logger.info(f'[{self.env.env_name} {self.name} {self.env.seed}][Export] Saved best source to {best_program_source_path}')
        checkpoint_path = best_program.get('checkpoint_path', '')
        if checkpoint_path and Path(checkpoint_path).exists():
            best_state_dict_path = f'{self.folder_path}best_state_dict.pt'
            shutil.copy2(checkpoint_path, best_state_dict_path)
            self.logger.info(f'[{self.env.env_name} {self.name} {self.env.seed}][Export] Saved best state_dict to {best_state_dict_path}')
