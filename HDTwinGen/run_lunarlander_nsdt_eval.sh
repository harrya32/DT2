...
ENV_ID="${ENV_ID:-LunarLanderContinuous-v3}"
POLICY_DIR="${POLICY_DIR:-pend-eval/lunarlander-policies}"
...
OUTPUT_JSON="${OUTPUT_JSON:-pend-eval/results/nsdt_lunarlander_alignment_summary.json}"
...
build_cmd() {
  ...
  local output_json="$2"
  local -a cmd=(
    "$PYTHON_BIN" "pend-eval/eval_nsdt_lunarlander_alignment.py"
    --env-id "$ENV_ID"
...
for ((offset=0; offset<SEED_RUNS; offset++)); do
  ...
  per_seed_json="${PER_SEED_OUTPUT_DIR}/nsdt_lunarlander_alignment_seed${seed}.json"
