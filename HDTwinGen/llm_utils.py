...
# Temporary local hardcoded Azure GPT-5 credentials (do not commit publicly)
GPT5_AZURE_KEY = "6lb9RMNtKtoP4WMWdB4T9FjPkTOmU3RhDGH4aXVNTXfremA1FLa2JQQJ99CCACL93NaXJ3w3AAABACOGWCG5"
GPT5_AZURE_API_BASE = "https://vdslabazuremloai-ae.openai.azure.com/"
GPT5_AZURE_API_VERSION = "2025-01-01-preview"


AZURE_MODEL_DETAILS_MAP = {
                            "gpt5-azure":   {'engine': "gpt-5-tl",
                                             'api_key': GPT5_AZURE_KEY,
                                             'api_version': GPT5_AZURE_API_VERSION,
                                             'api_base': GPT5_AZURE_API_BASE,
                                             'api_type': "azure",
                                             'properties': {
                                                 'model_name': 'gpt-5',
                                                 'model_version': '2025-01-01-preview',
                                                 'version_update_policy': 'Manual local configuration.',
...
def _model_supports_custom_sampling(model_name: str) -> bool:
    # Azure GPT-5 deployment used here only supports default sampling behavior.
    return model_name not in {"gpt5-azure"}

def get_llm_config(config, logger, name, rate_limiter):
    llm_config = {'model': config.run.model,
    ...
    if _model_supports_custom_sampling(config.run.model):
        llm_config['temperature'] = config.run.temperature
        llm_config['top_p'] = config.run.top_p
        llm_config['frequency_penalty'] = config.run.frequency_penalty
        llm_config['presence_penalty'] = config.run.presence_penalty
    # Some models (including Azure GPT-5 deployments) reject the stop parameter.
    if config.run.stop not in ("", None):
        llm_config['stop'] = config.run.stop
