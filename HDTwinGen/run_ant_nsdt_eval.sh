...
ENV_ID="${ENV_ID:-Ant-v5}"
POLICY_DIR="${POLICY_DIR:-pend-eval/ant-policies}"
POLICY_SNAPSHOTS="${POLICY_SNAPSHOTS:-}"
MAX_POLICIES="${MAX_POLICIES:-0}"

NSDT_RUN_DIR="${NSDT_RUN_DIR:-}"
LOGS_DIR="${LOGS_DIR:-logs}"
ENV_NAME="${ENV_NAME:-Dataset-Ant}"
ENV_SEED="${ENV_SEED:-0}"
...
ENV_KWARGS="${ENV_KWARGS:-{\"include_cfrc_ext_in_observation\": false}}"

OUTPUT_JSON="${OUTPUT_JSON:-pend-eval/results/nsdt_ant_alignment_summary.json}"
...
AGG_OUTPUT_JSON="${AGG_OUTPUT_JSON:-pend-eval/results/nsdt_ant_alignment_summary_all_seeds.json}"
...
build_cmd() {
  ...
  local output_json="$2"
  local -a cmd=(
    "$PYTHON_BIN" "pend-eval/eval_nsdt_ant_alignment.py"
    --env-id "$ENV_ID"
...
for ((offset=0; offset<SEED_RUNS; offset++)); do
  ...
  per_seed_json="${PER_SEED_OUTPUT_DIR}/nsdt_ant_alignment_seed${seed}.json"
