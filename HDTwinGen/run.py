...
def main(config, wandb, logger, rate_limiter):
    ...
    if df_results.empty or 'method' not in df_results.columns:
        logger.warning('[WARNING] No successful experiment rows with method field. Skipping table generation.')
        return
    if ('test_mse' not in df_results.columns) and ('reward' not in df_results.columns):
        logger.warning('[WARNING] No metric column found (expected test_mse or reward). Skipping table generation.')
        return
    if ('test_mse' in df_results.columns) and ('reward' not in df_results.columns):
        logger.info('[INFO] test_mse found without reward; skipping legacy reward table generation.')
        return
    try:
        tables = generate_main_results_table(df_results)
        logger.info(f'Tables: {tables}')
    except Exception as e:
        logger.warning(f'[WARNING] Failed generating summary tables: {e}. Skipping table generation.')
        return
    print('')
    # print(table_str)
    print('fin.')
...
def run_exp_wrapper_outer(args, **kwargs):
    ...
    if config.setup.debug_mode:
        ...
        try:
            ...
            traceback.print_exc()
            result = {'errored': True, 'method': method_name}
            print('')            
