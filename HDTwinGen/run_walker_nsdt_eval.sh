...
OUTPUT_JSON="${OUTPUT_JSON:-pend-eval/results/nsdt_walker_alignment_summary.json}"
...
AGG_OUTPUT_JSON="${AGG_OUTPUT_JSON:-pend-eval/results/nsdt_walker_alignment_summary_all_seeds.json}"
...
build_cmd() {
  ...
  local output_json="$2"
  local -a cmd=(
    "$PYTHON_BIN" "pend-eval/eval_nsdt_walker_alignment.py"
    --env-id "$ENV_ID"
...
for ((offset=0; offset<SEED_RUNS; offset++)); do
  ...
  per_seed_json="${PER_SEED_OUTPUT_DIR}/nsdt_walker_alignment_seed${seed}.json"
