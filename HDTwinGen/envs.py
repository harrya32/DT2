...
class TestGeneratedEnvEnvironment(Environment):
    ...
    def reset(self, config={}, logger=None):
        ...
        if self.env_name == 'covid-gcp':
            ...
            self.train_data, self.val_data, self.test_data, self.description = load_data(config=self.config, seed=self.seed, env_name=self.env_name)
            self.executor =  Executor(variables={'train_data': self.train_data,
                                                'val_data': self.val_data,
                                                'test_data': self.test_data,
                                        },
    ...
    def evaluate_simulator_code(self, StateDifferential, config={}, logger=None):
        ...
        if 'Dataset' in self.env_name:
            ...
            gt_env = DatasetEnv()
            return gt_env.evaluate_simulator_code_wrapper(StateDifferential=StateDifferential, train_data=self.train_data, val_data=self.val_data, test_data=self.test_data, config=config, logger=logger, env_name=self.env_name, env_seed=self.seed)
        elif 'COVID' in self.env_name:
