...
def make_sequence_dataset(dataset: OfflineDataset, seq_len: int, overlap: int = 0) -> OfflineDataset:
    """Convert flat transitions into fixed-length episode-respecting sequences with padding mask."""
    if seq_len <= 1:
        return dataset
...
def main() -> None:
    ...
    parser.add_argument("--dyn-seq-overlap", type=int, default=0, help="Overlap between consecutive sequences (0 = none)")
